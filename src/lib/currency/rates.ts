...
export function getCrossRate({
  ...
  // Cross-rate from the same date is ideal, but if dates differ (e.g. peg currencies with stale data) fall back to approximate division.
  if (rBase.date && rQuote.date && rBase.date !== rQuote.date) {
    console.warn(
      `getCrossRate: date mismatch for ${base}/${quote}`,
      rBase.date,
      rQuote.date,
    )
  }
  const crossRate = rQuote.rate / rBase.rate
  return crossRate
}
